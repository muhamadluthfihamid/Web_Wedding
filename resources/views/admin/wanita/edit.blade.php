...
@section('main-content')
    <!-- Page Heading -->
    <div class="mb-6">
        <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">{{ Auth::user()->isKhitanan() ? 'Edit Data Orang Tua' : 'Edit Biodata Wanita' }}</h1>
        <p class="text-sm text-slate-500 mt-1">{{ Auth::user()->isKhitanan() ? 'Perbarui informasi data orang tua ananda' : 'Perbarui informasi biodata pengantin wanita' }}</p>
    </div>
    ...
    <div class="max-w-3xl bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-100">
            <h3 class="text-lg font-bold text-slate-950">Edit Profil: {{ $biodataWanita->nama }}</h3>
        </div>
        <div class="p-6">
            <form action="{{ route('biodataWanita.update', $biodataWanita) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" id="nama" name="nama" value="{{ old('nama', $biodataWanita->nama) }}" required
                        ...
                        <div>
                            ...
                            <input type="text" id="bapak" name="bapak" value="{{ old('bapak', $biodataWanita->bapak) }}" required
                                   class="block w-full px-3 py-2.5 border border-slate-200 rounded-lg text-slate-950 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all">
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1" for="ibu">Nama Ibu<span class="text-rose-500">*</span></label>
                            <input type="text" id="ibu" name="ibu" value="{{ old('ibu', $biodataWanita->ibu) }}" required
                                   class="block w-full px-3 py-2.5 border border-slate-200 rounded-lg text-slate-950 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all">
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1" for="asal">Asal / Lokasi (Kota/Kabupaten)</label>
                            <input type="text" id="asal" name="asal" value="{{ old('asal', $biodataWanita->asal) }}"
                    ...
                    <div class="space-y-4">
                        ...
                        @if($biodataWanita->foto)
                            <div class="p-4 border border-slate-100 rounded-xl bg-slate-50 flex flex-col items-center gap-2">
                                ...
                                <img src="{{ asset('storage/' . $biodataWanita->foto) }}" alt="Foto Wanita" class="w-32 h-32 object-cover rounded-xl shadow-sm border border-slate-200">
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="4" required
                              class="block w-full px-3 py-2.5 border border-slate-200 rounded-lg text-slate-950 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all">{{ old('deskripsi', $biodataWanita->deskripsi) }}</textarea>
                </div>
