...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-slate-900">Tambah Paket Baru</h1>
</div>

<div class="max-w-2xl bg-white rounded-2xl border border-slate-100 shadow-sm p-8">
    <form action="{{ route('admin.rental.packages.store') }}" method="POST" class="space-y-5">
        @csrf
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="warna_badge" class="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                    ...
                    <option value="{{ $color }}" {{ old('warna_badge') === $color ? 'selected' : '' }}>{{ ucfirst($color) }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" name="durasi_hari" value="{{ old('durasi_hari') }}" required min="1"
                    class="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                    placeholder="90">
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="deskripsi" rows="2" class="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none" placeholder="Deskripsi singkat paket">{{ old('deskripsi') }}</textarea>
        </div>
        ...
        <div>
            ...
            <textarea name="fitur" rows="6" class="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm font-mono focus:ring-2 focus:ring-indigo-500 focus:outline-none" placeholder="Website undangan digital&#10;Halaman profil pengantin&#10;Countdown timer">{{ old('fitur') }}</textarea>
        </div>

        <div class="flex gap-6">
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_aktif" value="1" {{ old('is_aktif', '1') == '1' ? 'checked' : '' }}
                    class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                <span class="text-sm font-medium text-slate-700">Paket Aktif</span>
            </label>
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_populer" value="1" {{ old('is_populer') == '1' ? 'checked' : '' }}
