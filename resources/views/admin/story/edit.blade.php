...
@section('main-content')
    <!-- Page Heading -->
    <div class="mb-6">
        <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Edit Cerita Perjalanan Cinta</h1>
        <p class="text-sm text-slate-500 mt-1">Perbarui kisah perjalanan cinta Anda dalam 3 tahap penting</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden mb-8">
        <form action="{{ route('story.update', $story->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="bg-indigo-50/30 p-5 rounded-2xl border border-indigo-50/80">
                ...
                <textarea name="deskripsi" id="deskripsi" rows="3" required placeholder="Tuliskan kata-kata indah penutup cerita..."
            ...
            <div class="border-t border-slate-100 pt-6 flex flex-col sm:flex-row justify-between gap-4">
                ...
                <button type="submit" 
                    ...
                    <i class="fas fa-save"></i> Perbarui Cerita
                </button>
            </div>
        </form>
    </div>
