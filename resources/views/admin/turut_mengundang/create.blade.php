...
@section('main-content')
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Tambah Turut Mengundang</h1>
            <p class="text-sm text-slate-500 mt-1">Tambahkan nama keluarga, kerabat, atau tokoh yang turut mengundang</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm max-w-2xl">
        <div class="p-6 sm:p-8">
            <form action="{{ route('turutMengundang.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" name="hubungan" value="{{ old('hubungan') }}" 
                           placeholder="{{ Auth::user()->isKhitanan() ? 'Contoh: Paman / Keluarga Besar Ananda' : 'Contoh: Paman Pengantin Pria / Keluarga Besar Pengantin Wanita' }}"
                           class="block w-full px-3 py-2.5 border border-slate-200 rounded-lg text-slate-950 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all">
                    @error('hubungan')
                        <p class="mt-1 text-xs text-rose-500">{{ $message }}</p>
                    @enderror
