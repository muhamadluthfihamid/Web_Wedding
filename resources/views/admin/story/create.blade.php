...
@section('main-content')
    <!-- Page Heading -->
    <div class="mb-6">
        <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">{{ Auth::user()->isKhitanan() ? 'Tambah Ungkapan Harapan & Doa' : 'Tambah Cerita Perjalanan Cinta' }}</h1>
        <p class="text-sm text-slate-500 mt-1">{{ Auth::user()->isKhitanan() ? 'Tuliskan ungkapan bahagia dan doa untuk ananda khitan dalam 3 bagian' : 'Tulis kisah perjalanan cinta Anda dalam 3 tahap penting' }}</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden mb-8">
        <form action="{{ route('story.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-8">
            ...
            <div class="bg-indigo-50/30 p-5 rounded-2xl border border-indigo-50/80">
                ...
                <textarea name="deskripsi" id="deskripsi" rows="3" required placeholder="{{ Auth::user()->isKhitanan() ? 'Tuliskan ungkapan rasa syukur dan doa penutup dari keluarga...' : 'Tuliskan kata-kata indah penutup cerita perjalanan cinta Anda...' }}"
                ...
                <div class="bg-slate-50/50 p-5 rounded-2xl border border-slate-100 space-y-4">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <input type="text" name="judul_bertemu" required placeholder="{{ Auth::user()->isKhitanan() ? 'Contoh: Kehadiran Ananda Tercinta' : 'Contoh: Awal Pertemuan Kami' }}" value="{{ old('judul_bertemu') }}"
                        ...
                        <div>
                            ...
                            <textarea name="note_bertemu" rows="4" placeholder="{{ Auth::user()->isKhitanan() ? 'Ceritakan kebahagiaan saat ananda lahir atau masa awal kehadiran ananda...' : 'Ceritakan bagaimana Anda bertemu pertama kali...' }}"
                ...
                <div class="bg-slate-50/50 p-5 rounded-2xl border border-slate-100 space-y-4">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <input type="text" name="judul_serius" required placeholder="{{ Auth::user()->isKhitanan() ? 'Contoh: Tumbuh Menjadi Anak Sholeh' : 'Contoh: Berkomitmen Bersama' }}" value="{{ old('judul_serius') }}"
                                   class="mt-1 block w-full px-3 py-2 border border-slate-200 rounded-lg text-slate-950 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all">
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="note_serius" rows="4" placeholder="{{ Auth::user()->isKhitanan() ? 'Ceritakan momen tumbuh kembang atau kebaikan ananda sehari-hari...' : 'Ceritakan bagaimana hubungan kalian mulai mengarah ke arah yang serius...' }}"
                ...
                <div class="bg-slate-50/50 p-5 rounded-2xl border border-slate-100 space-y-4">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <input type="text" name="judul_tunangan" required placeholder="{{ Auth::user()->isKhitanan() ? 'Contoh: Momen Suci Khitanan' : 'Contoh: Hari Pertunangan Kami' }}" value="{{ old('judul_tunangan') }}"
                                   class="mt-1 block w-full px-3 py-2 border border-slate-200 rounded-lg text-slate-950 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all">
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="note_tunangan" rows="4" placeholder="{{ Auth::user()->isKhitanan() ? 'Tuliskan doa dan harapan orang tua saat melangkah ke momen khitanan...' : 'Ceritakan momen indah lamaran atau pertunangan Anda...' }}"
            ...
            <div class="border-t border-slate-100 pt-6 flex flex-col sm:flex-row justify-between gap-4">
                ...
                <button type="submit" 
                    ...
                    <i class="fas fa-save"></i> {{ Auth::user()->isKhitanan() ? 'Simpan Harapan & Doa' : 'Simpan Cerita' }}
                </button>
            </div>
        </form>
    </div>
